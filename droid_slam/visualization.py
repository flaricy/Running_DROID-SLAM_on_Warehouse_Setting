    ...
    def animation_callback(vis):
        ...
        with torch.no_grad():
            ...
            for i in range(len(dirty_index)):
                ...
                output_dir = "work/screen_capture"
                output_path = f"{output_dir}/frame_{ix:04d}.png"
...
def droid_visualization_headless(video, output_dir, device="cuda:0"):
    """ DROID visualization frontend (headless version) """
    ...
    droid_visualization.filter_thresh = 0.1 # <original> 0.005

    def save_frames():
        """ Process and save frames without visualization window """
        with torch.no_grad():
            with video.get_lock():
                dirty_index, = torch.where(video.dirty.clone())
                dirty_index = dirty_index
            ...
            print(f'len(dirty_index) = {len(dirty_index)}')
            disps = disps.cpu()
            masks = ((count >= 1)) # & (disps > 0.01 * disps.mean(dim=[1, 2], keepdim=True))) # <original> disps > 0.5 * ...
            ...
            output_path = f"{output_dir}/all_frames.png"
